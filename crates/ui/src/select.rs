...
#[component]
pub fn Select<T: std::clone::Clone + std::cmp::PartialEq + std::fmt::Display + 'static>(
	values: Vec<T>, current_value: T, callback: EventHandler<T>, label: Option<String>,
) -> Element {
          ...
          for value in values {
            div {
              ...
              Icon {
                ...
                class: tw_join!(
                    "fill-none ml-auto", if value == current_value { "opacity-100" } else {
                    "opacity-0" }
                )
              }
            }
