...
impl ToastManager {
	pub fn new(maximum_toast: u8) -> Self {
		Self { list: BTreeMap::new(), maximum_toast, id_manager: ID::new() }
	}
...
impl ToastInfo {
	pub fn simple(text: &str) -> Self {
		Self { heading: None, context: text.to_string(), allow_toast_close: true, position: Position::BottomLeft, icon: None, hide_after: Some(6) }
	}
...
pub fn ToastFrame(props: ToastFrameProps) -> Element {
	...
	for (id, item) in toast_list.iter() {
		...
		let element = rsx! {
			div {
				class: "block w-full p-2.5 mb-1 rounded font-sans text-xs leading-[17px] relative pointer-events-auto bg-gray-700 text-white {icon_class}",
				id: "{id}",
