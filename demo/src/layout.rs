...
pub fn Layout(children: Element) -> Element {
  ...
  let backdrop = rsx! {
    div {
      class: tw_join!(
        ...
        if menu_open() { "opacity-100" } else { "opacity-0 pointer-events-none" }
      ),
    ...
    nav {
      ...
      class: tw_join!(
        ...
        if current_route.name() == "Home" { 
          "hidden lg:hidden translate-x-0"
        } else if ! menu_open() {
          "hidden xl:block"
        } else { 
  ...
  let menu_toggle_button = rsx! {
    ...
    div {
      class: "dark bg-gray-900 flex flex-col relative h-screen min-h-screen",
      ...
      header {
        ...
        a {
          ...
          Icon { icon: FiGithub, width: 16, height: 16, class: "sm:w-16 sm:h-16 w-8 h-8" }
