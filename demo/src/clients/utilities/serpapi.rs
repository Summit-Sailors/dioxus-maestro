...
fn SerpApiDemo() -> Element {
	...
	let on_search = move |query: String| {
		...
		async move {
			...
			match search_google(query).await {
				Ok(results) => {
					state.set(SearchState { query: state().query.clone(), results, loading: false, error: None });
				},
				Err(e) => {
					state.set(SearchState { query: state.query.clone(), results: Vec::new(), loading: false, error: Some(format!("Error: {}", e)) });
				},
			}
		}
	};
	...
	let on_query_change = move |evt: Event<FormData>| {
		to_owned![state];
		state.set(SearchState { query: evt.value.clone(), results: state.results.clone(), loading: state.loading, error: state.error.clone() });
	};
