...
pub async fn add_email_job(to: String, subject: String, body: String) -> Result<(), ServerFnError> {
	...
	let mut storage = apalis_storage_from_ctx::<apalis_example::EmailJob>().await?;

	let job = apalis_example::EmailJob { to, subject, body };
...
pub async fn list_pending_jobs() -> Result<String, ServerFnError> {
	use maestro_apalis::server_ctx::{Storage, apalis_storage_from_ctx};
	let storage = apalis_storage_from_ctx::<apalis_example::EmailJob>().await?;
...
#[server]
pub async fn send_email(job: apalis_example::EmailJob) -> Result<(), ServerFnError> {
