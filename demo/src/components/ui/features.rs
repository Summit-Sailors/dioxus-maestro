...
use {
  dioxus::prelude::*,
  dioxus_free_icons::{icons::{fa_solid_icons::FaCheck, io_icons::IoChevronForward}, Icon},
...
pub fn Features(props: FeatureProps) -> Element {
  ...
  rsx! {
    div {
      class: "mb-4 rounded-lg hover:shadow-lg",
      ...
      div {
        ...
        span { 
          class: "font-semibold text-md text-gray-100",
        ...
        span { 
          Icon {
            ...
            class: tw_join!(
              "transition-transform duration-200 text-gray-50",
              (is_expanded()).then_some("rotate-180")
            ),
          }
        }
      }
      ...
      div {
        class: tw_join!(
          "px-3 pb-3 w-full",
          if is_expanded() {"block"} else {"hidden"}
        ),
        
        div {
          class: "mt-2 p-4 rounded-lg shadow-lg",
          ul {
            {props.features.iter().map(|feature| {
              ...
              rsx! {
                li {
                  class: "flex flex-wrap items-start mb-2 space-x-2",
                  ...
                  if !_feature.is_empty() {
                    span { class: "text-gray-100 font-medium border rounded-md border-gray-700 p-1", "{_feature}" }
