...
pub fn ParallelQueriesDemo() -> Element {
	...
	rsx! {
		div { 
      class: "flex flex-col items-center justify-center rounded-lg shadow-lg p-6 bg-gray-900",
			div { 
        class: "w-full max-w-4xl bg-gray-900 rounded-lg shadow-lg p-6",
				h3 { class: "text-2xl text-gray-100 text-center font-bold mb-6",
					"Parallel Queries"
				}
                    ...
                    .map(|dept| {
                      ...
                      rsx! {
                        div {
                          key: dept.id.clone(),
                          class: tw_join!(
                            "p-4 rounded-lg border border-gray-800 transition-colors cursor-pointer", 
                            if is_selected {"border-blue-500" } else { "bg-gray-800" }
                          ),
              ...
              QueryResult::Err(_) => rsx! {
                ...
                QueryResult::Ok(emps) => rsx! {
                  {
                    emps.iter()
                      .map(|emp| {
                        let emp = emp.clone();
                        rsx! {
                          div { key: emp.id.clone(), class: "p-4 bg-gray-800 rounded-lg border border-gray-800 shadow-sm",
				...
				div { class: "grid flex justify-center gap-4 mt-6",
					Button {
						...
						onclick: move |_| {
              query_client.invalidate_queries(&[String::from("departments"), String::from("employees")]);
            },
						"Refresh All"
					}
