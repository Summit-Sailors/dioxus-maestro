use {
    ...
    std::fmt::Error, tailwind_fuse::tw_join,
  };
  ...
  pub fn ParallelQueriesDemo() -> Element {
      ...
      let employees_query = use_get_query(
          [
              ...
              selected_dept().unwrap_or_default(),
          ],
          move |_| async move {
      ...
      rsx! {
          div {
              class: "grid grid-cols-1 md:grid-cols-2 gap-4 mb-4 p-4 bg-white rounded-lg shadow-lg",
              
              div {  
                  class: "space-y-4",
                  h2 { class: "text-xl font-bold text-center text-gray-800", "Department Overview" }
                  ...
                  div {  
                      ...
                      match departments_query.result().value() {
                          QueryResult::Loading(_) => rsx! {
                              div { class: "col-span-3 text-center text-gray-500", "Loading departments..." }
                          },
                                  ...
                                  rsx! {
                                      div {
                                          ...
                                          class: tw_join!(
                                              ...
                                              if is_selected {"bg-blue-50 border-blue-500"} else {"bg-white hover:bg-gray-50"}
                                          ),
                                          onclick: move |_| selected_dept.set(Some(dept.id.clone())),
  
                                          h3 { class: "font-semibold text-lg", "{dept.name}" }
                                          p { class: "text-sm text-gray-600", "Location: {dept.location}" }
              ...
              div {  
                  class: "space-y-4",
                  h2 { class: "text-xl font-bold text-center text-gray-800", "Department Employees" }
                  ...
                  if selected_dept().is_some() {
                      div {
                          ...
                          match employees_query.result().value() {
                              QueryResult::Loading(_) => rsx!{
                                  div { class: "text-center text-gray-500", "Loading employees..." }
                              },
                                      ...
                                      {emps.iter().map(|emp| {
                                          ...
                                          rsx! {
                                              div {
                                                  ...
                                                  div {
                                                      class: "space-y-2",
                                                      h4 { class: "font-semibold", "{emp.name}" }
                                                      p { class: "text-sm text-gray-600", "{emp.title}" }
                                                      p { class: "text-sm text-gray-600", 
                                                          "Salary: ",
                              ...
                              QueryResult::Err(_) => rsx!{
                                  div { class: "text-center text-red-500", "Error loading employees" }
                              }
                          }
                      }
                  } else {
                      div { class: "text-center text-gray-500", "Select a department to view employees" }
                  }
              }
  
              div { 
                  class: "grid flex grid-cols-1 md:grid-cols-2 gap-2 justify-center",
                  Button {
                  ...
                  if let Some(dept) = selected_dept() {
                    {println!("selected department: {}", dept)}
                      Button {
                          class: "bg-green-500 text-white rounded hover:bg-green-600",
                          on_click: move |_| query_client.invalidate_queries(&["employees".to_string(), dept.clone()]),
