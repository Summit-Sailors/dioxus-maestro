...
pub fn ParallelQueriesDemo() -> Element {
	...
	rsx! {
		div { class: "flex flex-col items-center justify-center rounded-lg shadow-lg p-6 bg-gray-100",
			div { class: "w-full max-w-4xl bg-white rounded-lg shadow-lg p-6",
				h3 { class: "text-2xl text-gray-700 text-center font-bold mb-6",
													...
													.map(|dept| {
															...
															rsx! {
																div {
																	key: dept.id.clone(),
																	class: tw_join!(
																			"p-4 rounded-lg border transition-colors cursor-pointer", if is_selected {
																			"bg-blue-50 border-blue-500" } else { "bg-white hover:bg-gray-50" }
																	),
									...
									QueryResult::Ok(emps) => rsx! {
										{
												emps.iter()
														.map(|emp| {
																let emp = emp.clone();
																rsx! {
																	div { key: emp.id.clone(), class: "p-4 bg-white rounded-lg border shadow-sm",
				...
				div { class: "grid flex justify-center gap-4 mt-6",
					Button {
						class: "bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600",
						onclick: move |_| query_client.invalidate_query(String::from("departments")),
						"Refresh All Data"
					}
					if let Some(dept) = selected_dept() {
						Button {
							class: "bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600",
							onclick: move |_| query_client.invalidate_queries(&["employees".to_string(), dept.clone()]),
