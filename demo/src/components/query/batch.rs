...
impl DisplayData for User {
  ...
  fn get_content(&self) -> String { self.email.clone() }
  fn get_metadata(&self) -> Vec<(String, String)> {
...
impl DisplayData for Comment {
  fn get_title(&self) -> String { format!("Comment on {}", self.post_id) }
  fn get_content(&self) -> String { self.content.clone() }
  fn get_metadata(&self) -> Vec<(String, String)> {
...
pub fn BatchOperationsDemo() -> Element {
  ...
  let mock_users = vec![
    User {
      id: "u1".into(),
      name: "Steve".into(),
      email: "steve.smith@example.com".into(),
      role: "Admin".into(),
    },
    User {
      id: "u2".into(),
  ...
  let mock_comments = vec![
    Comment {
      ...
      post_id: "p1".into(),
      author_id: "u2".into(),
      content: "Great introduction to Rust!".into(),
      likes: 15,
    },
    Comment {
      id: "c2".into(),
      post_id: "p2".into(),
      author_id: "u1".into(),
      content: "Very helpful patterns explanation.".into(),
      likes: 12,
    },
  ];

  let users_query = use_get_query(
  ...
  let posts_query = use_get_query(
      ["posts-batch".to_string()],
      move |_|{ 
  ...
  let comments_query = use_get_query(
      ["comments-batch".to_string()],
      move |_| {
    ...
    div { 
      ...
      div { 
        ...
        Button {
          ...
          on_click: handle_batch_mutation,
          "Run Batch Mutation"
        }
      }
    }
