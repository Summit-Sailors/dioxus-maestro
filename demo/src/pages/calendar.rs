...
pub fn CalendarDemo() -> Element {
	...
	rsx! {
		div {
			h1 { class: "text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 dark:text-gray-50 mb-8 text-center",
			...
			ComponentSection {
				...
				div { class: "p-2 space-y-2 space-x-1",
					Button {
						class: "px-4 py-2 border text-gray-500 border-gray-800 rounded-lg shadow-sm hover:shadow-md transition hover:bg-gray-200",
						onclick: move |_| {
			...
			ComponentSection {
				...
				div { class: "flex flex-col items-center w-full",
					...
					div { class: "mt-4 space-y-2 text-center w-full",
						h3 { class: "text-lg font-semibold text-gray-700", "Upcoming Events:" }
						{
								events
										.read()
										.iter()
										.map(|event| {
												rsx! {
													div { class: "flex flex-wrap items-center text-center space-x-2 text-gray-800 justify-center",
														div {
