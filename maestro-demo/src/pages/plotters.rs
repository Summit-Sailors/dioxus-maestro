...
use maestro_plotters::{
	...
	pie::{PieChartOptions, use_pie_chart},
	radar::use_radar,
...
pub fn PlottersDemo() -> Element {
	...
	let pie_sizes = Memo::new(move || use_explicit_memo((), || Some(vec![35, 25, 20, 15, 5])));

	let pie_labels = Memo::new(move || {
		use_explicit_memo((), || {
			Some(vec!["Product A".to_string(), "Product B".to_string(), "Product C".to_string(), "Product D".to_string(), "Others".to_string()])
		})
	});
	...
	let chart_options = ChartOptions::builder()
		...
		.series_labels(vec!["Current Year".into(), "Previous Year".into(), "Target".into()])
		.colors(vec![RGBColor(65, 105, 225), RGBColor(46, 139, 87), RGBColor(255, 99, 71), RGBColor(255, 215, 0), RGBColor(148, 0, 211)])
		.font(("Arial".into(), 24))
		.build();
	...
	let pie_colors = vec![RGBColor(65, 105, 225), RGBColor(46, 139, 87), RGBColor(255, 99, 71), RGBColor(255, 215, 0), RGBColor(148, 0, 211)];
	...
	let scatter_highlights = Memo::new(move || use_explicit_memo((), || Some(vec![(4.2, 3.8), (8.3, 7.1), (12.4, 11.2)])));
	...
	let heatmap_data = Memo::new(move || {
		use_explicit_memo((), || Some(vec![vec![0.8, 0.2, 0.6, 0.4], vec![0.3, 0.7, 0.5, 0.9], vec![0.5, 0.4, 0.8, 0.2], vec![0.4, 0.6, 0.3, 0.7]]))
	});

	let heatmap_labels = Memo::new(move || use_explicit_memo((), || Some(vec!["Q1", "Q2", "Q3", "Q4"])));
				...
				Features {
					...
					features: vec![
							"Superior Type Safety: Leverages Rust's type system for chart configurations and data validation"
									.to_string(),
							"Enhanced Error Handling: Built-in integration with maestro-toast for graceful error handling"
									.to_string(),
							"Reactive Architecture: Uses Dioxus hooks for automatic updates and efficient rendering"
									.to_string(),
							"Unified Configuration: Consistent API across all chart types through the ChartOptions system"
									.to_string(),
							"Canvas Integration: Direct canvas backend integration for optimal performance"
									.to_string(),
							"Memory Management: Use Memo::new for data that doesn't need frequent updates"
									.to_string(),
							"Performance: Share ChartOptions instances when possible for multiple charts"
									.to_string(),
					],
				}
			}
