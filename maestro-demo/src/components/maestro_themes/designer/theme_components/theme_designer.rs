...
#[derive(Clone)]
pub struct DesignerStateManager {
	state: Signal<DesignerState>,
}

impl DesignerStateManager {
	pub fn new(initial_state: DesignerState) -> Self {
		Self { state: Signal::new(initial_state) }
	}

	pub fn get(&self) -> DesignerState {
		self.state.read().clone()
	}

	pub fn apply_action(&mut self, action: ThemedesignerAction) {
		let mut current_state = self.state.read().clone();
...
pub fn ThemeDesigner(props: ThemeDesignerProps) -> Element {
	...
	let state = use_context_provider(|| Signal::new(designer_state.unwrap_or_default()));

	let mut active_tab = use_signal(|| "colors");

	let mut current_component_group = use_signal(|| components_id_clone.clone());
	...
	let mut theme_options = use_signal(|| ThemeOptions { with_doc_themes: with_doc_theme(), format: export_format(), components_id });

	// editor actions
	let reset_theme = move |_| {
		state().apply_action(ThemedesignerAction::ResetToDefaults);
	};
	...
	use_effect({
		let comp_id = components_id_clone.clone();
		move || {
			theme_options.set(ThemeOptions { with_doc_themes: with_doc_theme(), format: export_format(), components_id: comp_id.clone() });
		}
	});
      ...
      div { class: "theme-designer-sidebar col-span-1 bg-[color:var(--card-bg)] p-6 rounded-lg border border-[color:var(--border-color)] shadow-md overflow-y-auto w-full md:w-1/2",
        ...
        div { class: "component-group-selector mb-6 p-4 bg-[color:var(--bg-color)] rounded-[var(--radius-md)]",
          ...
          Select {
            ...
            onchange: move |value| current_component_group.set(value),
