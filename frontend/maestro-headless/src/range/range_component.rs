...
pub struct RangeContext {
	...
	pub values: Memo<Option<Vec<f32>>>,
	pub set_values: Callback<Option<Vec<f32>>>,
...
pub struct RangeProps {
	...
	#[props(default = None)]
	pub on_value_change: Option<Callback<Option<Vec<f32>>>>,
...
pub fn Range(props: RangeProps) -> Element {
	...
	let callback = use_callback(move |values: Option<Vec<f32>>| {
	...
	let update_values = use_callback(move |(new_value, at_index): (f32, usize)| {
		let current_values = values().clone().unwrap_or_default();
		...
		if has_min_steps_between_values(&next_values, min_steps_between_thumbs * step) {
			...
			if has_changed {
				set_values(Some(next_values.clone()));
			}
		}
	});

	let handle_slide_start = use_callback(move |new_value: f32| {
		let closest_index = get_closest_value_index(&values().unwrap_or_default(), new_value);
			...
			onkeydown: move |event: KeyboardEvent| {
					if !disabled() {
							match event.key() {
									...
									Key::End => {
											update_values((max, values().unwrap_or_default().len() - 1));
											event.prevent_default();
									}
									Key::ArrowUp
									| Key::ArrowRight
...
pub fn RangeTrack(props: RangeTrackProps) -> Element {
	let context = use_context::<RangeContext>();
	let percentages =
		context.values.read().clone().unwrap_or_default().iter().map(|&value| convert_value_to_percentage(value, context.min, context.max)).collect::<Vec<f32>>();
	let offset_start =
		if context.values.read().clone().unwrap_or_default().len() > 1 { percentages.iter().cloned().reduce(f32::min).unwrap_or(0.0) } else { 0.0 };
...
pub fn RangeThumb(props: RangeThumbProps) -> Element {
	...
	let percent = match *context.orientation.read() {
		EOrientation::Horizontal =>
			context.values.read().clone().unwrap_or_default().get(index() as usize).map(|v| convert_value_to_percentage(*v, context.min, context.max)).unwrap_or(0.0),
		EOrientation::Vertical =>
			context.values.read().clone().unwrap_or_default().get(index() as usize).map(|v| convert_value_to_percentage(*v, context.max, context.min)).unwrap_or(0.0),
	};

	let label = get_label(index() as usize, context.values.read().clone().unwrap_or_default().len());
	...
	rsx! {
		div {
			...
			aria_valuenow: context.values.read().clone().unwrap_or_default().get(index() as usize).cloned(),
			...
			display: if context.values.read().clone().unwrap_or_default().get(index() as usize).is_none() { "none" } else { "flex" },
...
fn linear_scale(input: [f32; 2], output: [f32; 2]) -> impl Fn(f32) -> f32 {
	move |value: f32| {
...
fn get_decimal_count(value: f32) -> usize {
	value.to_string().split('.').nth(1).map_or(0, |s| s.len())
}

fn round_value(value: f32, decimal_count: usize) -> f32 {
	let rounder = 10.0_f32.powi(decimal_count as i32);
	(value * rounder).round() / rounder
}

fn get_next_sorted_values(prev_values: &[f32], next_value: f32, at_index: usize) -> Vec<f32> {
...
fn convert_value_to_percentage(value: f32, min: f32, max: f32) -> f32 {
...
fn get_label(index: usize, total_values: usize) -> String {
	if total_values > 2 {
...
fn get_closest_value_index(values: &[f32], next_value: f32) -> usize {
	if values.len() == 1 {
		0
	} else {
...
fn get_thumb_in_bounds_offset(width: f32, left: f32) -> f32 {
...
fn get_steps_between_values(values: &[f32]) -> Vec<f32> {
	values.windows(2).map(|w| w[1] - w[0]).collect()
}

fn has_min_steps_between_values(values: &[f32], min_steps_between_values: f32) -> bool {
