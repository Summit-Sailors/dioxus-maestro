...
pub fn Select<T: Clone + PartialEq + std::fmt::Display + 'static>(props: SelectProps<T>) -> Element {
	...
	let display_value = current_value
		.as_ref()
		.and_then(|value| options.iter().find(|opt| &opt.value == value).map(|opt| opt.label.clone()))
	...
	rsx! {
		div { class: tw_merge!("flex flex-col gap-2 w-full relative min-h-12", & props.container_class),
			...
			div {
				class: tw_merge!(
					"relative w-full cursor-pointer maestro-select-button__wrapper", & props.button_wrapper_class
				),
				button {
					class: tw_merge!(
					...
					span {
						class: tw_merge!(
							...
							if is_placeholder { placeholder_class } else { "" }
						),
						"{display_value}"
					}
					{icon_down}
				...
				div {
					...
					if props.is_searchable {
						div {
							class: tw_merge!(
								"relative px-3 text-gray-500 maestro-select-search_container", & props.search_input_container_class
							),
							Icon {
								width: 16,
								icon: BsSearch,
