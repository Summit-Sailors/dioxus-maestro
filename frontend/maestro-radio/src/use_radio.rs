	...
	Channel: RadioChannel<Value>,
{
	pub(crate) fn is_listening(&self, channel: &Channel, scope_id: &ScopeId) -> bool {
		...
		listeners.get(scope_id).map(|listener| &listener.channel == channel).unwrap_or_default()
	}
	...
	pub(crate) fn listen(&self, channel: Channel, scope_id: ScopeId) {
		dioxus_lib::prelude::warnings::signal_write_in_component_body::allow(|| {
			...
			listeners.insert(scope_id, RadioListener { channel, drop_signal: CopyValue::new_maybe_sync(()) });
		});
	}
	...
	Channel: RadioChannel<Value>,
{
	...
	pub fn write(&mut self) -> RadioGuard<Value, Channel> {
		...
		RadioGuard { channels: self.antenna.peek().get_channel().derive_channel(&*value), antenna: self.antenna, value }
	}
	...
	pub fn write_channel(&mut self, channel: Channel) -> RadioGuard<Value, Channel> {
		let value = self.antenna.peek().station.value.write_unchecked();
		RadioGuard { channels: channel.derive_channel(&*value), antenna: self.antenna, value }
	}
	...
	Value: 'static,
{
	...
	let radio = use_hook(|| Radio::new(Signal::new(RadioAntenna::builder().station(station).scope_id(current_scope_id().unwrap()).build())));
	radio.antenna.peek().station.listen(channel, current_scope_id().unwrap());
	radio
}

pub fn use_init_radio_station<Value, Channel>(init_value: impl FnOnce() -> Value) -> RadioStation<Value, Channel>
where
	Channel: RadioChannel<Value>,
	Value: 'static,
{
