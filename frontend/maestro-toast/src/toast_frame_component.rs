...
use crate::{toast_code::EToastCode, toast_manager::ToastManager, toast_position::EToastPosition};
...
pub fn ToastFrame(mut manager: Signal<ToastManager>) -> Element {
	...
	for (id, item) in manager.read().list.iter() {
		...
		let (icon_element, color_classes, progress_color) = if let Some(icon) = &item.info.icon {
			match icon {
				EToastCode::Success => (
					rsx! {
						Icon { width: 16, height: 16, icon: BsCheckCircleFill }
					},
		...
		let total_duration = item.hide_after;
		...
		let element = rsx! {
			div {
				...
				id: "{id}",
				div { class: "flex items-center gap-1.5 mb-1 w-full",
					div { class: tw_join!("flex-shrink-0", & item.info.icon.is_none().then_some("hidden")),
						{icon_element}
					}
					if let Some(v) = &item.info.heading {
						h2 { class: "font-sans text-sm font-medium bg-none inherit leading-inherit flex-grow",
							"{v}"
						}
					}
	...
	rsx! {
		div { class: "toast-scope",
			div {
				class: "fixed pointer-events-none m-0 p-0 z-[9999] bottom-5 left-5 flex flex-col items-start",
				id: "wrap-bottom-left",
				style: "isolation: isolate;",
				{bottom_left_ele.into_iter()}
			}
			div {
				class: "fixed pointer-events-none m-0 p-0 z-[9999] bottom-5 right-5 flex flex-col items-end",
				id: "wrap-bottom-right",
				style: "isolation: isolate;",
				{bottom_right_ele.into_iter()}
			}
			div {
				class: "fixed pointer-events-none m-0 p-0 z-[9999] top-5 left-5 flex flex-col items-start",
				id: "wrap-top-left",
				style: "isolation: isolate;",
				{top_left_ele.into_iter()}
			}
			div {
				class: "fixed pointer-events-none m-0 p-0 z-[9999] top-5 right-5 flex flex-col items-end",
				id: "wrap-top-right",
				style: "isolation: isolate;",
...
async fn time_sleep(_interval: usize) {
	#[cfg(feature = "web")]
	gloo_timers::future::TimeoutFuture::new(_interval as u32).await;

	#[cfg(feature = "desktop")]
	tokio::time::sleep(tokio::time::Duration::from_millis(_interval as u64)).await;
}
